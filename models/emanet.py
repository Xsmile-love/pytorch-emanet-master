...
class EMANet(nn.Module):
    def __init__(self, block, layers, num_classes=10):
        ...
        self.layer1 = self._make_layers(block, 64, layers[0], stride=1, groups=[1, 4, 8, 16])
        self.layer2 = self._make_layers(block, 128, layers[1], stride=2, groups=[1, 4, 8, 16])
        self.layer3 = self._make_layers(block, 256, layers[2], stride=2, groups=[1, 4, 8, 16])
        self.layer4 = self._make_layers(block, 512, layers[3], stride=2, groups=[1, 4, 8,16])
...
def emanet101():
    model = EMANet(EMABlock, [3, 4, 23, 3], num_classes=10)
    return model
